...
export class StringPipe implements PipeTransform {
  ...
  private checkIntegerOptionValue(optionName: string, value: number | undefined) {
    ...
    if (!this.validateIsNonNegativeInteger(value)) throw new Error(`${optionName} should be an non-negative integer`);
    return value;
  }
  ...
  private check(name: string, value: string, ErrorType: Type<Error>) {
    if (typeof this.minLength === 'number' && value.length < this.minLength) {
      throw new ErrorType(`The length of ${name} should be greater than or equal to ${this.minLength}`);
    }

    if (typeof this.maxLength === 'number' && value.length > this.maxLength) {
      throw new ErrorType(`The length of ${name} should be less than or equal to ${this.maxLength}`);
    }
...
export const UuidPipe = new StringPipe({ minLength: 36, maxLength: 36 });
